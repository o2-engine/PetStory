...
CLASS_FIELDS_META(LevelController)
{
    ...
    FIELD().PRIVATE().SERIALIZABLE_ATTRIBUTE().SERIALIZABLE_ATTRIBUTE().NAME(mGoals);
    FIELD().PRIVATE().NAME(mCollected);
    FIELD().PRIVATE().DEFAULT_VALUE(false).NAME(mCompletedFired);
    FIELD().PRIVATE().SERIALIZABLE_ATTRIBUTE().SERIALIZABLE_ATTRIBUTE().DEFAULT_VALUE(0).NAME(mMovesLimit);
