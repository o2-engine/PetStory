...
// manager with the meta and gameplay screens and shows the start
// screen. The scene update drives the manager, so the game runs
// wherever the scene runs. Removal from scene tears everything down.
// ------------------------------------------------------------------
...
{
    FIELD().PRIVATE().EDITOR_PROPERTY_ATTRIBUTE().SERIALIZABLE_ATTRIBUTE().DEFAULT_VALUE(String("Levels/Chain.json")).NAME(mChainPath);
    FIELD().PRIVATE().EDITOR_PROPERTY_ATTRIBUTE().SERIALIZABLE_ATTRIBUTE().DEFAULT_VALUE(String("Meta")).NAME(mStartScreen);
